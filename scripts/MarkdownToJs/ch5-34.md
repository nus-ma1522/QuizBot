...
When we add $u_2$ to the basis, the line expands into a plane as it is able to explore a second dimension now.
...
$v_1$ is quite straightforward. We can let $v_1 = u_1$.
...
How many non-zero entries must the vector $r_1$ have?
...
Now, suppose that we have some vector $r_2$, such that $a_2 = Qr_2$.

By similar logic as above, $a_2$ is dependent only on $q_1, q_2$. Hence, $r_2$ can only have non-zero entries in the first two rows, and everything else must be zero.
...
Instead, we can make use of the fact that $\mathbf{Q}^T\mathbf{Q} = I$. Premultiplyng by $Q^{T}$ on both sides, we have $Q^{T}A = Q^{T}QR = R$.

Hence, to get $R$, we just need to multiply $Q^{T}$ and $A$! Pretty simple right?
